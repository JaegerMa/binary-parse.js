...
function create(binaryParse)
{
	...
	let parsers =
	{
		...
		float: (that) => _.readFloat(that),
		floatbe: (that) => _.readFloat(that),
		floatle: (that) => _.readFloat(that, {endian: 'le'}),
		double: (that) => _.readDouble(that),
		doublebe: (that) => _.readDouble(that),
		doublele: (that) => _.readDouble(that, {endian: 'le'}),
		string: (that, length, encoding) => _.readString(that, { length, encoding }),
		utf8String: (that, length) => _.readString(that, { length, encoding: 'utf8' }),
		utf16LEString: (that, length) => _.readString(that, { length, encoding: 'utf16le' }),
		base64String: (that, length) => _.readString(that, { length, encoding: 'base64' }),
		hexString: (that, length) => _.readString(that, { length, encoding: 'hex' }),
